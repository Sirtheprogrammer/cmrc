...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!csrf_verify()) throw new RuntimeException('Invalid CSRF token');
        $username = $_POST['username'] ?? '';
        $password = $_POST['password'] ?? '';
        $phone = $_POST['phone'] ?? null;
        $email = $_POST['email'] ?? null;

        $user = user_register($pdo, $username, $password, $phone, $email);
...
<head>
  ...
  <title>Register — Lupyana Tech</title>
        ...
        <div class="card-body">
          ...
          <form method="post" novalidate>
            ...
            <div class="d-flex justify-content-between align-items-center">
              <a href="<?php echo $base; ?>/login.php">Already have an account?</a>
